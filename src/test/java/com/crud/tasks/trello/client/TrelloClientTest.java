...
class TrelloClientTest {
    ...
    public void shouldCreateCard() throws URISyntaxException {
        ...
        URI uri = new URI(
                "http://test.com/cards?key=test&token=test&name=Test%20task&desc=test%20desc&pos=top&idList=test_id");
    ...
    @Test
    public void doesReturnEmptyList() throws URISyntaxException {
        //given
        when(trelloConfig.getTrelloApiEndpoint()).thenReturn("http://test.com");
        when(trelloConfig.getTrelloAppKey()).thenReturn("test");
        when(trelloConfig.getTrelloToken()).thenReturn("test");
        ...
        //when
        List<TrelloBoardDto> boardsToBeNull = trelloClient.getTrelloBoard();

        //then
        assertEquals(0, boardsToBeNull.size());
